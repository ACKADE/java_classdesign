...
public final class DatabaseInitializer {
    ...
    public static void initialize() {
        try (Connection serverConnection = DBUtil.getServerConnection();
             Statement statement = serverConnection.createStatement()) {

            statement.executeUpdate("CREATE DATABASE IF NOT EXISTS text DEFAULT CHARACTER SET utf8mb4");
            statement.executeUpdate("USE text");

            statement.executeUpdate("CREATE TABLE IF NOT EXISTS cs_students ("
            ...
            statement.executeUpdate("CREATE TABLE IF NOT EXISTS cs_teacher ("
            ...
            seedStudents(statement);
            seedTeachers(statement);
        } catch (SQLException e) {
            throw new RuntimeException("数据库初始化失败：" + e.getMessage(), e);
        }
    }

    private static void seedStudents(Statement statement) throws SQLException {
        statement.executeUpdate("INSERT IGNORE INTO cs_students (student_id, major, grade, dorm_address, birth_date) VALUES "
                + "('202411010101', '计算机科学与技术', '2024级', '杏园公寓2幢303室', '2006-03-15'),"
    ...
    private static void seedTeachers(Statement statement) throws SQLException {
        statement.executeUpdate("INSERT IGNORE INTO cs_teacher (teacher_id, name, major, title, phone, email, office_address, hire_date) VALUES "
